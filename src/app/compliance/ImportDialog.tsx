      ...
      reader.onload = async (e) => {
        try {
          ...
          jsonData.forEach((row: any) => {
            ...
            if (!regulationsMap.has(title)) {
              regulationsMap.set(title, {
                title: title,
                regulation_type: row.Type || row.Tipe || row.regulation_type || "POJK",
                category: row.Category || row.Kategori || row.category || "Internal",
                issued_date: formatDate(row.Date || row.Tanggal || row.issued_date),
          ...
          for (let i = 0; i < regulations.length; i++) {
            ...
            setProgress(`Mengimpor: ${reg.title} (${i + 1}/${regulations.length})`);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" className="h-11 px-6 border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-all shadow-sm gap-2">
          <Upload size={18} />
          Import CSV/XLSX
        </Button>
      </DialogTrigger>
          ...
          <div className="space-y-6">
            <div 
              className={`border-2 border-dashed rounded-3xl p-10 text-center transition-all ${
                file ? 'border-blue-200 bg-blue-50/30' : 'border-slate-100 bg-slate-50/50 hover:bg-slate-50 hover:border-slate-200'
              }`}
            >
              <input 
                type="file" 
            ...
            {loading && (
              <div className="bg-blue-50/50 p-4 rounded-2xl flex items-center gap-3 animate-pulse">
                ...
                <p className="text-xs font-bold text-blue-700 uppercase tracking-widest">{progress}</p>
              </div>
            )}
            ...
            <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
               ...
               <ul className="space-y-2">
                  <li className="text-[11px] font-medium text-slate-600 flex items-start gap-2">
                    ...
                    <span>Gunakan kolom <strong>Title</strong> sebagai kunci pengelompokan regulasi.</span>
                  </li>
                  <li className="text-[11px] font-medium text-slate-600 flex items-start gap-2">
                    <div className="w-1.5 h-1.5 bg-blue-400 rounded-full mt-1 flex-shrink-0" />
                    <span>Kolom <strong>ItemRef</strong> dan <strong>ItemContent</strong> wajib diisi untuk tiap baris item.</span>
                  </li>
               </ul>
            </div>
          </div>
