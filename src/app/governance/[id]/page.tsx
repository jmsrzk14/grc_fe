...
import { ArrowLeft, Edit, Trash2, Info, Activity } from "lucide-react";
    ...
    <div className="space-y-6 animate-in fade-in duration-500 w-full px-2 pb-10">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <HeaderTitle title={`Detail Governance: ${id}`} />
        </div>
        <div className="flex items-center gap-2">
           ...
           <Button onClick={() => toast({ title: "Sesi Modifikasi Aktif", description: "Anda sedang memasuki mode edit data secara real-time." })} className="h-9 px-4 bg-primary text-primary-foreground rounded-md text-sm font-medium hover:bg-primary/90 transition-all shadow-sm">
              <Edit className="h-4 w-4 mr-1.5" /> Edit Data
           </Button>
        </div>
      </div>
        ...
        <div className="space-y-6 w-full">
          <Card className="border border-border shadow-sm bg-card rounded-xl overflow-hidden">
            <CardHeader className="bg-muted/20 border-b border-border px-6 py-4">
              <CardTitle className="text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">Informasi Utama</CardTitle>
            </CardHeader>
            <CardContent className="p-6 space-y-6">
               <div className="grid grid-cols-2 gap-6">
                 <div className="bg-slate-50/50 p-4 rounded-lg border border-slate-100">
                   <p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-1">ID Data</p>
                   <p className="text-sm font-mono font-bold text-slate-700">{id}</p>
                 </div>
                 <div className="bg-slate-50/50 p-4 rounded-lg border border-slate-100">
                   <p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-1">Status Publikasi</p>
                   <p className="text-sm font-bold text-emerald-600">Terjadwal (Scheduled)</p>
                 </div>
                 <div className="col-span-2 bg-slate-50/50 p-4 rounded-lg border border-slate-100">
                   <p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-1.5">Deskripsi / Kontekstual Info</p>
                   <p className="text-sm text-slate-600 leading-relaxed font-medium">Halaman detail ini merepresentasikan informasi komprehensif terkait ID {id}. Integrasi database dinamis akan secara otomatis me-load elemen yang bersangkutan pada fase koneksi Backend API.</p>
                 </div>
               </div>
            </CardContent>
          </Card>
