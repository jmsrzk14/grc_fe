...
import { Plus, Search, Filter, ShieldAlert, ChevronLeft, ChevronRight, AlertCircle, Calendar } from "lucide-react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import HeaderTitle from "@/components/layout/HeaderTitle";
import { RISK_REGISTER } from "@/lib/data";
...
export default function RiskPage() {
  ...
  const filteredData = RISK_REGISTER.filter(item =>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 w-full px-2 pb-10">
      <HeaderTitle title="Risk Management" />
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex gap-2 items-center">
          <form className="relative" onSubmit={(e) => e.preventDefault()}>
            ...
            <input
              ...
              placeholder="Cari ID, Judul Risiko, atau PIC..."
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="h-9 px-3 bg-white border border-slate-200 text-slate-600 rounded-md text-sm font-medium hover:bg-slate-50 transition-all flex items-center gap-1.5 flex-none">
            <ShieldAlert className="w-4 h-4 text-muted-foreground" /> Mitigasi
          </button>
          <Button size="sm" asChild className="h-9 px-4 bg-primary text-primary-foreground rounded-md text-sm font-medium hover:bg-primary/90 transition-all flex items-center gap-1.5 shadow-sm flex-none">
            <Link href="/risk/create">
              <Plus className="w-4 h-4" /> Identifikasi
            </Link>
          </Button>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="pl-6 py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">ID Risiko</TableHead>
              <TableHead className="py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">Nama Risiko</TableHead>
              <TableHead className="py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">Kategori | Impact</TableHead>
              <TableHead className="text-center py-4 text-xs font-bold uppercase tracking-widest text-muted-foreground">Residual Score</TableHead>
          ...
          <TableBody>
            {paginatedData.length > 0 ? (
              paginatedData.map((item) => (
                <TableRow 
                  key={item.id} 
                  onClick={() => router.push(`/risk/${item.id}`)}
                ...
                >
                  ...
                  <TableCell className="py-4 text-sm font-medium text-muted-foreground tracking-tight">
                    {item.category} <span className="mx-2 text-border">|</span> {item.impact}
                  </TableCell>
                  <TableCell className="py-4 text-center">
                    <span className={`text-xs font-semibold px-2 py-1 rounded border ${
                      item.residualScore > 10 ? 'bg-rose-50 text-rose-600 border-rose-100' :
                      item.residualScore > 5 ? 'bg-amber-50 text-amber-600 border-amber-100' :
                  ...
                  <TableCell className="py-4 text-center pr-6">
                    <div className="inline-flex items-center gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full ${item.status === 'Mitigated' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.4)]' : 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.4)]'}`} />
                      <span className={`text-xs font-medium tracking-widest ${item.status === 'Mitigated' ? 'text-emerald-600' : 'text-rose-600'}`}>
                        {item.status}
                      </span>
                    </div>
                  </TableCell>
